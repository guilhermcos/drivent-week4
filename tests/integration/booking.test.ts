...
describe('Update /booking/:bookingId', () => {
  ...
  describe('when token is valid', () => {
    it('should respond with status code 200 and change the booking', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      const ticket = await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      const hotel = await createHotel();
      const roomFirst = await createRoomWithHotelId(hotel.id);
      const roomSecond = await createRoomWithHotelId(hotel.id);
      const booking = await createBooking(user, roomFirst);

      const body = {
        roomId: roomSecond.id,
      };

      const response = await server.put(`/booking/${booking.id}`).set('Authorization', `Bearer ${token}`).send(body);
      const newBooking = await findBookingWithRoomInfo(booking);

      expect(response.status).toBe(httpStatus.OK);
      expect(newBooking.Room.id).toBe(roomSecond.id);
      expect(response.body).toEqual(
        expect.objectContaining({
